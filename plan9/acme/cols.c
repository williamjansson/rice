...
colinit(Column *c, Rectangle r)
{
	...
	r1.min.y = r1.max.y;
	r1.max.y += Border;
	draw(screen, r1, allocimagemix(display, DPalebluegreen, DWhite), nil, ZP);
...
coladd(Column *c, Window *w, Window *clone, int y)
{
	...
	if(c->nw > 0){
		...
		draw(screen, r1, allocimage(display, Rect(0,0,1,1), screen->chan, 1, DSand), nil, ZP);
...
colresize(Column *c, Rectangle r)
{
	...
	for(i=0; i<c->nw; i++){
		...
		draw(screen, r2, allocimagemix(display, DPalebluegreen, DWhite), nil, ZP);
		r1.min.y = r2.max.y;
...
colgrow(Column *c, Window *w, int but)
{
  ...
  Found:
	...
	for(j=0; j<i; j++){
		...
		r.max.y += Border;
		draw(screen, r, allocimage(display, Rect(0,0,1,1), screen->chan, 1, DSand), nil, ZP);
		y1 = r.max.y;
	}
