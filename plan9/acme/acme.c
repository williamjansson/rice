...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil) {
		...
		tagcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xEEEEDA00);
		tagcols[TEXT] = display->black;
		tagcols[HTEXT] = display->black;

		/* Yellow */
		textcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DSand);
		textcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DDarkyellow);
		textcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DSand);
		textcols[TEXT] = display->black;
		textcols[HTEXT] = display->black;
	}

	r = Rect(0, 0, Scrollwid+ButtonBorder, font->height+0);
	...
	border(button, r, ButtonBorder, tagcols[BACK], ZP);

	r = button->r;
	modbutton = allocimage(display, r, screen->chan, 0, DNofill);
	draw(modbutton, r, tagcols[BACK], nil, r.min);
	r.max.x -= ButtonBorder;
	border(modbutton, r, ButtonBorder, tagcols[BACK], ZP);
	...
	r = button->r;
	colbutton = allocimagemix(display, DPalebluegreen, DWhite);
